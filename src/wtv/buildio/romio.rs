...
impl BuildIO for ROMIO {
	...
	fn open(file_path: String, stripped: Option<bool>, rom_size: Option<u32>) -> Result<Option<ROMIO>, Box<dyn std::error::Error>> {
		let mut wtvbuild_io = ROMIO {
	...
	fn create(file_path: String, stripped: Option<bool>, rom_size: Option<u32>) -> Result<Option<ROMIO>, Box<dyn std::error::Error>> {
		let mut wtvbuild_io = ROMIO {
			stripped: stripped.unwrap_or(false),
			rom_size: rom_size.unwrap_or(0x000000),
			file_path: file_path.clone(),
